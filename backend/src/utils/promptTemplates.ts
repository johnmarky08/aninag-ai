...
export function buildAnalyzePrompt(text: string, sources: RawSource[]) {
  return `
You are an evidence-focused verifier. Your task is to analyze the given text using ONLY the provided sources. For each source, use its snippet to check if the content supports, contradicts, or is unrelated to the claim, and then enrich the sources with additional metadata.

CRITICAL RULES:
- Step 1: Use the provided sources array. Do not invent or fabricate URLs. Do not add or remove entries.
- Step 2: For each source, determine its bias (e.g., "News", "Fact-check", "Education") and whether it contradicts the claim (true/false) based on the snippet.
